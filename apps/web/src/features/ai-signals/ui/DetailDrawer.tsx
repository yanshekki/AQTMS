...
import { Drawer, Box, Typography, IconButton, Stack, Chip, Divider, CircularProgress, Card, CardContent, LinearProgress } from '@mui/material';
...
export function DetailDrawer({ signalId, onClose }: DetailDrawerProps) {
  ...
  // Extract top-level suggested action if available
    ...
    >
      <Box sx={{ p: { xs: 2, sm: 3 } }}>
        ...
        {detail && (
          <>
            {/* Content */}
            <Typography variant="body1" sx={{ color: isDark ? '#d1d5db' : '#334155', mb: 2, lineHeight: 1.7, fontSize: '0.95rem' }}>
              {detail.content}
            </Typography>
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                          ...
                          {r.result.urgency && (
                            <Chip
                              ...
                              size="small"
                              sx={{
                                bgcolor: r.result.urgency === 'CRITICAL' ? '#ef444420' : r.result.urgency === 'HIGH' ? '#f59e0b20' : chipBg,
