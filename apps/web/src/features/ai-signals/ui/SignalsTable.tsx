...
export const SignalsTable = memo(function SignalsTable({ signals, isLoading, error, onSelectSignal }: SignalsTableProps) {
  ...
  if (error) return <Box py={4} textAlign="center"><Typography sx={{ color: '#ef4444' }}>Error: {error}</Typography></Box>;
  if (signals.length === 0) return (
    <Box sx={{ textAlign: 'center', py: 8, bgcolor: cardBg, borderRadius: 3, border: 1, borderColor }}>
      <Typography variant="h6" sx={{ color: mutedText, mb: 1 }}>No AI Signals Yet</Typography>
      <Typography variant="body2" sx={{ color: isDark ? '#374151' : '#cbd5e1' }}>Connect data sources (Telegram / X.com) and AI providers.</Typography>
    </Box>
  );
      ...
      <Table size="small">
        <TableHead>
          <TableRow sx={{ bgcolor: isDark ? '#0f172a' : '#f1f5f9' }}>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor, display: { xs: 'none', sm: 'table-cell' } }}>Time</TableCell>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor }}>Source</TableCell>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor }}>Content</TableCell>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor, textAlign: 'center', display: { xs: 'none', md: 'table-cell' } }}>Flag</TableCell>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor, textAlign: 'center' }}>Score</TableCell>
            <TableCell sx={{ color: mutedText, fontWeight: 600, fontSize: '0.7rem', borderColor, display: { xs: 'none', lg: 'table-cell' } }}>T/S/R</TableCell>
