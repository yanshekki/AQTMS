...
export function ConnectExchangeModal({ open, onClose, onConnect, isConnecting, connectError, testConnection }: ConnectExchangeModalProps) {
  ...
  return (
    <Dialog open={open} onClose={isConnecting ? undefined : onClose} maxWidth="sm" fullWidth
      ...
      <DialogTitle sx={{ color: isDark ? '#f3f4f6' : '#0f172a', fontWeight: 700, borderBottom: 1, borderColor }}>{t('exchanges.connectExchange')}</DialogTitle>
      <DialogContent sx={{ pt: 3 }}>
        {connectError && <Alert severity="error" sx={{ mb: 2, borderRadius: 2 }}>{connectError.includes('invalid') || connectError.includes('permission') ? t('exchanges.apiKeyInvalid') : connectError}</Alert>}
      ...
      <DialogActions sx={{ px: 3, pb: 2, borderTop: 1, borderColor, pt: 2 }}>
        <Button onClick={onClose} disabled={isConnecting} sx={{ color: isDark ? '#6b7280' : '#94a3b8' }}>{t('exchanges.cancel')}</Button>
